rm -rf ~/gpu-ec2/docker/web-ui
cd ~/gpu-ec2/docker
git clone https://github.com/browser-use/web-ui.git
cd web-ui
cp .env.example .env

EIP_PUBLIC_DNS=$(aws ssm get-parameter --name "/myowu/info" --query "Parameter.Value" --output text | jq -r ".eipPublicDnsG")
...
OPENAI_API_KEY=$(aws ssm get-parameter --name "/myowu/info" --query "Parameter.Value" --output text 2>/dev/null | jq -r ".liteLLMApiKey" 2>/dev/null)
...
# Add the extra_hosts configuration before environment
